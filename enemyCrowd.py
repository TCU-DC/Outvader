...
class EnemyCrowd:
    ...
    def __init__(self, crowdStartX, crowdStartY):
        '''
        初期化
        '''
        ...
        # 敵軍の配列初期化
        self.enemyList = [[Enemy((self.singleSize + 5)*i + crowdStartX, j * (self.singleSize + 5) +
                                 crowdStartY, (i * ENEMY_X_MAX + j) % 3) for i in range(ENEMY_X_MAX)] for j in range(ENEMY_Y_MAX)]
    ...
    def update(self, panelw, panelh, mt):
        ...
        if pyxel.frame_count % 20 == 0:
            rx = random.randrange(ENEMY_X_MAX)
            ry = random.randrange(
                ENEMY_Y_MAX)
            # 生存していれば...ね
            if self.enemyList[ry][rx].isAlive:
                rpos = self.enemyList[ry][rx].getPos()
